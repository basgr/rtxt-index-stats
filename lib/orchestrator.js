...
async function doRun(host, { force, confirmed, keepCache }) {
  ...
  // Batch all rows into ONE message. MV3 service workers drop rapid
  // sendMessage bursts (200+ calls in tight sync loop = most messages
  // lost). Live per-row updates during fetching still use run:row,
  // but they're spaced out by the 10-30s throttle so no burst risk.
  broadcast({ type: 'run:rows', host, rows });
...
function replayState(host, state) {
  ...
  broadcast({ type: 'run:rows', host, rows: state.rules });
