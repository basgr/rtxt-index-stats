/**
 * Parse a robots.txt body and return the Disallow + Allow rules that apply
 * to Googlebot and to the wildcard "*" User-agent block.
 *
 * Output shape:
 *   {
 *     googlebot: { disallows: string[], allows: string[] },
 *     wildcard:  { disallows: string[], allows: string[] }
 *   }
 *
 * Empty values are dropped. Sitemap / Crawl-delay / Host and comments
 * are ignored.
 *
  ...
  for (const rawLine of lines) {
    ...
    if (directive === 'user-agent') {
      ...
      if (!current || current.hasBody) {
        current = { agents: [], disallows: [], allows: [], hasBody: false };
        groups.push(current);
      }
      ...
      // Sitemap / Crawl-delay / Host / unknowns: not stored, but they do
      // seal the group so the next User-agent starts a new one.
      if (current) current.hasBody = true;
    }
  }

  const out = {
    googlebot: { disallows: [], allows: [] },
    wildcard:  { disallows: [], allows: [] },
  };
  for (const group of groups) {
    if (group.agents.includes('googlebot')) {
      out.googlebot.disallows.push(...group.disallows);
      out.googlebot.allows.push(...group.allows);
    }
    if (group.agents.includes('*')) {
      out.wildcard.disallows.push(...group.disallows);
      out.wildcard.allows.push(...group.allows);
    }
  }
  return out;
}
...
function canonicalDirective(raw) {
  ...
  if (ALLOW_TYPOS.has(d)) return 'allow';
  return d;
}
